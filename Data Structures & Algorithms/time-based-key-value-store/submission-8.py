class TimeMap:
    ...
    def set(self, key: str, value: str, timestamp: int) -> None:
        #insertion sort
        index = 0
        for idx, val in enumerate(self.hashmap[key]):
            timestamp_prev, _ = val
            if timestamp_prev > timestamp:
                index = idx
                break
            if idx == len(self.hashmap[key]) - 1:
                index = len(self.hashmap[key])
        
        if index == len(self.hashmap[key]):
            self.hashmap[key].append((timestamp, value))
        else:
            self.hashmap[key].insert(index, (timestamp, value))
